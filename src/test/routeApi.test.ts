...
describe("routeApi diagnostics", () => {
  ...
  it("normaliza rota com separadores e sufixo de país", () => {
    ...
    expect(normalizeRouteLabel("Canoas | RS ; Brasil")).toBe("canoas, rs");
  });
  ...
  it("usa cache quando rota já existe e atualiza telemetria no intervalo", async () => {
    maybeSingleMock.mockResolvedValue({
      data: { id: "cache-1", distance_km: 450, hit_count: 3, last_used_at: null },
      error: null,
    });
  ...
  it("evita write no cache hit quando last_used_at é recente", async () => {
    maybeSingleMock.mockResolvedValue({
  ...
  it("permite refresh manual forçando provider", async () => {
    vi.mocked(invokeEdgeFunction).mockResolvedValue({
      distanceKm: 1110,
      originCoords: { lat: -29.918, lon: -51.179 },
      destCoords: { lat: -23.55, lon: -46.633 },
    ...
    const result = await refreshRouteCacheEntry({
      origin: "Canoas - RS",
      destination: "São Paulo - SP",
      userId: "user-1",
    });
